...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: cs => (makeCodeTree(left, right) :: cs ) sortWith (weight(_) < weight(_))
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def walk(remaining: CodeTree, bits: List[Bit]): List[Char] = remaining match {
      case Leaf(c, _) if bits.isEmpty => List(c)
      case Leaf(c, _) => c :: walk(tree, bits)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def walk(tree: CodeTree)(c: Char): List[Bit] = tree match {
      case Leaf(_, _) => List()
      case Fork(left, right, _, _) if chars(left).contains(c) => 0 :: walk(left)(c)
      case Fork(left, right, _, _) => 1 :: walk(right)(c)
    }

    text flatMap walk(tree)
  }
   ...
   */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table.toMap.getOrElse(char, List())

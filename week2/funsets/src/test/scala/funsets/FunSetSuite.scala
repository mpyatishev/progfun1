...
class FunSetSuite extends FunSuite {
  ...
  test("diff contains all elements that are not in second set") {
    new TestSets {
  ...
  test("filter returns all elements that match predicate") {
    new TestSets {
      val s = filter(union(s1, s2), (x: Int) => x > 1)
  ...
  test("forall returns whether all elements satisfies p") {
    new TestSets {
      val s = union(union(s1, s2), s3)
      val r1 = forall(s, (x: Int) => x >= 1)
      val r2 = forall(s, (x: Int) => x >= 2)
      val r3 = forall(s, (x: Int) => x != 3)
  ...
  test("exists returns whether any elements satisfies p") {
    new TestSets {
      val s = union(union(s1, s2), s3)
      val r1 = exists(s, (x: Int) => x >= 1)
